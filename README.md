...
| 量 | 值 | 为什么 |
|---|---|---|
| 正文栏 | 48rem / 768px | ÷17px ≈ 45 字/行。中文 40~46 字最舒服，英文那套 45~75 字符照搬会挤 |
| 图区 | 68rem / 1088px | 容得下两张图并排 + 间距 |
| 图内正文级字 | 15px | 正文 17px 的 0.88。**图里的文字是标注不是阅读流**，惯例就该小一号 |
| 图内最小字 | ≥ 12px | 图注级 |

每张图显示多宽由图内字号反推，不用手工指定：

```
显示宽 = 15px × 原图宽 / (图内正文级字号pt × dpi/72)
```

这样每张图的「正文级」文字贴到页面上都是 15px，彼此一致、和正文也成比例。
**不能统一 pt、也不能统一显示宽**：各图画幅从 1000px 到 1950px 不等，同一个 pt
缩放后差 1.6 倍；统一显示宽又让不同画幅的图字号各异。只有反推才能对齐。

「正文级字号」取按字符数加权的中位数——标题只有一行、主标签有几十个，
直接取中位数会被零散的大字拉偏。

这个数由 `save_web()` 写进图片同目录的 `_web.json`，`build.py` 读了原样用。
**页面自己另算就破坏了对齐。**

### 并排

md 里紧挨着的两张图（中间没有正文），宽度和放得下就自动并成一行，顶对齐。
略超页面宽时按比例缩一点塞进去，缩到 88% 为止——那时图内字 13.2px，还能读，
比让两张图各占一屏强。再超就各自独占。
