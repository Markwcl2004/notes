...
页面上每张图显示多宽是算出来的，不用手工指定：

```
显示宽 = 原图宽 × 0.47，夹到 [460, 960]
```

**为什么按原图宽等比例算，而不是分几档**：各张图的画幅从 1000px 到 1950px 不等，
如果显示宽只有「正文栏 / 挣脱」两档，缩放比例会差出 1.6 倍 —— 同一个 pt 字号，
有的图上 15px、有的 40px，一篇文章里的插图看着就不是一套。按固定比例缩，
图内字的像素大小才处处相同。副作用正好是想要的：内容多的图自然更大。

出图时 `save_web()` 按同一个宽度兜底字号（图内最小字 ≥ 15px，正文 17px），
`build.py` 把算出的宽度写进 `<figure style="--w:...">`。**两边公式必须一致**，
改一边就要改另一边，否则那条 15px 底线在页面上不成立。

超过正文栏 60px 的图会挣脱居中，其余留在正文栏内；md 里标了 `|N`（N ≤ 700）的按最小宽显示。
